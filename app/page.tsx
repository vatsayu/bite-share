...
import { ArrowRight , Users, Coins, MapPin,  HandPlatter, Utensils } from 'lucide-react'
...
function AnimatedGlobe() {
  return (
    <div className="relative w-32 h-32 mx-auto mb-8">
      ...
      <HandPlatter className="absolute inset-0 m-auto h-16 w-16 text-green-600 animate-pulse"/></div>
  )
}
  ...
  useEffect(() => {
    async function fetchImpactData() {
      try {
        ...
        const WeFeedWeCare = wasteCollected ; 

        setImpactData({
          wasteCollected: Math.round(wasteCollected * 10) / 10, // Round to 1 decimal place
          reportsSubmitted,
          tokensEarned,
          WeFeedWeCare: Math.round(WeFeedWeCare * 10) / 10 // Round to 1 decimal place
        });
      } catch (error) {
    ...
    <div className={`container mx-auto px-4 py-16 ${poppins.className}`}>
      <section className="text-center mb-20">
        ...
        <h1 className="text-6xl font-bold mb-6 text-gray-800 tracking-tight">
          Bite & Share <span className="text-green-600"></span>
        </h1>
        <h2 className='text-2xl font-bold tracking-tight'> Don't <span className="text-red-500">Trash</span> It, <span className="text-green-500">Share</span> It!</h2>

        <p className="text-xl text-gray-600 max-w-2xl mx-auto leading-relaxed mb-8">
          Join our community in making food waste management more efficient and rewarding!
        </p>
        {!loggedIn ? (
      ...
      <section className="grid md:grid-cols-3 gap-10 mb-20">
        <FeatureCard
          icon={HandPlatter}
          title="Food for All"
          description="Contribute to a needer or hunger by reporting and collecting waste."
        />
        <FeatureCard
          icon={Coins}
          title="Earn Rewards"
          description="Get tokens for your contributions to food  waste management efforts."
        />
        <FeatureCard
          icon={Users}
      ...
      <section className="bg-white p-10 rounded-3xl shadow-lg mb-20">
        ...
        <div className="grid md:grid-cols-4 gap-6 ">
          <ImpactCard title="Waste Collected" value={`${10} kg`} icon={Utensils} />  
          ...
          <ImpactCard title="We Feed, We Care" value={impactData.WeFeedWeCare.toString()} icon={HandPlatter} />
        </div>
       
      </section>
     
       <div className='flex-col  text-black-400 text-center py-4 mb-2  rounded-2xl'>
        <p className='mb-2 '>Tel: 1800-00-000 Email: support@BiteShare.com | Copyright © 2024 Bite&Share | All Rights Reserved.</p>
          <p>Website developed by: @yush | Kartik | Akshita | Manvi</p>
        </div>
